...
class Message:
    ...
    def send(self):
        ...
        try:
            with StatusChangeMessageBrokerService() as broker:
...
class StatusChangeMessageBrokerService:
    RABBITMQ_EXCHANGE = 'genesis.task.status'
    ...
    def __init__(self, rabbitmq_url=settings.RABBITMQ_URL):
        ...
        self.channel.queue_declare(
            queue=settings.STATUS_CHANGE_RABBITMQ_TITLE, durable=True
        )
        self.channel.queue_bind(
            exchange=self.RABBITMQ_EXCHANGE,
            queue=settings.STATUS_CHANGE_RABBITMQ_TITLE, routing_key=''
        )
